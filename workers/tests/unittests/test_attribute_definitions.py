import pytest

from ohsome_quality_analyst.attributes import definitions
from ohsome_quality_analyst.attributes.models import Attribute


@pytest.fixture()
def attribute_key():
    return "height"


def test_get_attributes():
    atbs = definitions.get_attributes()
    assert isinstance(atbs, dict)
    for key, value in atbs.items():
        for k, v in value.items():
            assert isinstance(k, str)
            assert isinstance(v, Attribute)


def test_get_attribute(attribute_key, topic_key_building_count):
    atb = definitions.get_attribute(topic_key_building_count, attribute_key)
    assert isinstance(atb, Attribute)


def test_get_attribute_wrong_key():
    with pytest.raises(KeyError):
        definitions.get_attribute("foo", "bar")
